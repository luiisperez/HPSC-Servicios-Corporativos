...
AS
BEGIN
	INSERT INTO EQUIPO_SERVICIO VALUES (@eqsv_id, @eqsv_fechaini, @eqsv_fechafin, @eq_serial, @sv_id);
END
...
AS
BEGIN
	SELECT ES.EC_ID AS ID, S.SV_NIVELSERVICIO AS NIVELSERV, S.SV_TIPOSERVICIO AS TIPOSERV, S.SV_TIEMPORESPUESTA AS TIEMPORESP, 
...
/****** 
		PROCEDIMIENTO PARA ELIMINAR UN SERVICIO DE UN EQUIPO
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[ELIMINAR_SERVICIO_ASIGNADO]
  @eqsv_id AS varchar(500)
AS
BEGIN
	SELECT ES.EC_ID AS ID, S.SV_NIVELSERVICIO AS NIVELSERV, S.SV_TIPOSERVICIO AS TIPOSERV, S.SV_TIEMPORESPUESTA AS TIEMPORESP, 
		   ...
		   ES.EC_FECHAINICIO AS INICIO, ES.EC_FECHAFIN AS FECHAFIN, S.SV_ID AS IDSERVICIO
	FROM SERVICIO S, EQUIPO E, EQUIPO_SERVICIO ES
	WHERE ES.EC_FK_EQUIPO = E.EQ_SERIAL AND S.SV_ID = ES.EC_FK_SERVICIO AND EC_ID = @eqsv_id;
END
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[MODIFICAR_SERVICIO_ASIGNADO]
  @sv_id AS varchar(500),
  @eqsv_id AS varchar(500),
  @eqsv_fechaini as date,
  @eqsv_fechafin as date
AS
BEGIN
	UPDATE EQUIPO_SERVICIO SET EC_FK_SERVICIO = @sv_id, EC_FECHAFIN = @eqsv_fechafin, EC_FECHAINICIO = @eqsv_fechaini WHERE EC_ID = @eqsv_id;
END
